...
resource "aws_iam_instance_profile" "search_profile" {
    name = "search-profile"
...
resource "aws_iam_role_policy_attachment" "service_role_ssm" {
    role       = aws_iam_role.search_role.name
    policy_arn = aws_iam_policy.search_deployment_s3.arn
}
resource "aws_iam_policy" "search_deployment_s3" {
    name        = "search-source-s3-policy"
    description = "deployment S3 access for search"
