...
resource "aws_iam_role_policy_attachment" "service_role_ssm" {
    role       = aws_iam_role.web_catalogue_role.name
    policy_arn = aws_iam_policy.web_catalogue_deployment_s3.arn
}
resource "aws_iam_policy" "web_catalogue_deployment_s3" {
    name        = "web-catalogue-source-s3-policy"
    description = "deployment S3 access for web catalogue"      
    policy = templatefile("${path.module}/templates/deployment-source-s3-access.json", {
        deployment_s3_bucket = var.deployment_s3_bucket,
        service              = "web-catalogue"
    })
}
