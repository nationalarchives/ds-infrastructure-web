...
resource "aws_iam_role_policy_attachment" "service_role_ssm" {
    role       = aws_iam_role.wagtail_role.name
    policy_arn = aws_iam_policy.wagtail_deployment_s3.arn
}
resource "aws_iam_policy" "wagtail_deployment_s3" {
    name        = "wagtail-source-s3-policy"
    description = "deployment S3 access for wagtail"
