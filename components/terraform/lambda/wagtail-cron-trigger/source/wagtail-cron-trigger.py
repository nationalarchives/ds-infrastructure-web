...
def wagtail_cron_trigger(event, context):
    ...
    if task == 'publish_scheduled':
        cron_command = f"sudo docker-compose -f /var/docker/compose.yml exec -T {active_container} poetry run python /app/manage.py publish_scheduled"
    elif task == 'clearsessions':
        cron_command = f"sudo docker-compose -f /var/docker/compose.yml exec -T {active_container} poetry run python /app/manage.py clearsessions"
    else:
        return {
            'statusCode': 400,
