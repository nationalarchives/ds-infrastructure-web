...
then
  ...
  image_id=$(sudo docker image ls --quiet ghcr.io/nationalarchives/ds-hospital-records:$docker_image_tag)
  if [[ $image_id != '' ]]
  then
    ...
    # remove image
    sudo docker rmi ghcr.io/nationalarchives/ds-hospital-records:$docker_image_tag
  else
    # do  nothing
  ...
  # pull image
  sudo docker image pull ghcr.io/nationalarchives/ds-hospital-records:$docker_image_tag
  # get image id
  image_id=$(sudo docker image ls --quiet ghcr.io/nationalarchives/ds-hospital-records:$docker_image_tag)
  # run container
