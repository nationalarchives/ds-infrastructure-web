resource "aws_cloudfront_origin_access_control" "download" {
  name                              = "ds-${var.environment}-downloads.s3.eu-west-2.amazonaws.com"
  description                       = "OAC for merlin downloads S3 bucket"
...
resource "aws_cloudfront_distribution" "web" {
  ...
  # ------------------------------------------------------------
  dynamic "origin" {
    for_each = local.cloudfront_distribution.s3_origins

    content {
      domain_name = origin.value["domain_name"]
      origin_id   = origin.value["origin_id"]

      origin_access_control_id = aws_cloudfront_origin_access_control.download.id
    }
  }
  ...
  default_cache_behavior {
    ...
    allowed_methods = lookup(
      local.cloudfront_distribution,
      "default_behaviour_allowed_methods",
      ""
    )

    cached_methods = lookup(
      local.cloudfront_distribution,
      "default_behaviour_cached_methods",
      ""
    )
    ...
    viewer_protocol_policy = lookup(
      local.cloudfront_distribution,
      "behaviour_default_viewer_protocol_policy",
      ""
    )
