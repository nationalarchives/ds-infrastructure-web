...
for full_name in $PARAM_NAMES; do
    ...
    if [ "$key" == "docker_images" ]; then
        ...
        CATALOGUE_APP_IMAGE=$(echo "$docker_json" | jq -r '.["catalogue-application"]')
        TRAEFIK_IMAGE=$(echo "$docker_json" | jq -r '.["traefik"]')

        printf 'DOCKER_IMAGE_CATALOGUE_APPLICATION="%s"\n' "$CATALOGUE_APP_IMAGE" >> "$OUTPUT_FILE"
...
# Pull the image
sudo docker pull "$CATALOGUE_APP_IMAGE"
...
# Update app version
if [ "$CATALOGUE_APP_IMAGE" != "$set_app_image" ]; then
  sudo yq -i ".services.blue-web.image = \"$CATALOGUE_APP_IMAGE\"" /var/docker/compose.yml
  sudo sed -i "s|export CATALOGUE_APP_IMAGE=.*|export CATALOGUE_APP_IMAGE=\"$CATALOGUE_APP_IMAGE\"|g" /etc/environment
fi
