...
PARAMETER_PATH="/application/web/forms"
if [ -z ${TRAEFIK_IMAGE+x} ]; then export TRAEFIK_IMAGE="none"; fi
if [ -z ${FORMS_APP_IMAGE+x} ]; then export FORMS_APP_IMAGE="none"; fi

# Install dependencies
sudo dnf -y update && sudo dnf install -y aws-cli jq

OUTPUT_FILE="/var/docker/forms.env"
...
for full_name in $PARAM_NAMES; do
    ...
    if [ "$key" == "docker_images" ]; then
        ...
        FORMS_APP_IMAGE=$(echo "$docker_json" | jq -r '.["forms-application"]')
        TRAEFIK_IMAGE=$(echo "$docker_json" | jq -r '.["traefik"]')

        printf 'DOCKER_IMAGE_FORMS_APPLICATION="%s"\n' "$FORMS_APP_IMAGE" >> "$OUTPUT_FILE"
...
# Pull the image
sudo docker pull "$FORMS_APP_IMAGE"
...
# update app version
if [ "$FORMS_APP_IMAGE" != "$set_app_image" ]; then
  sudo yq -i ".services.blue-web.image = \"$FORMS_APP_IMAGE\"" /var/docker/compose.yml
  sudo sed -i "s|export FORMS_APP_IMAGE=.*|export FORMS_APP_IMAGE=\"$FORMS_APP_IMAGE\"|g" /etc/environment
fi
