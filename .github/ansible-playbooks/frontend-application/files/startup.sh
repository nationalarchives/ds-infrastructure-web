...
# Pull the image
sudo docker pull "$FRONTEND_APP_IMAGE"
...
# update app version
if [ "$FRONTEND_APP_IMAGE" != "$set_app_image" ]; then
  sudo yq -i ".services.blue-web.image = \"$FRONTEND_APP_IMAGE\"" /var/docker/compose.yml
  sudo sed -i "s|export FRONTEND_APP_IMAGE=.*|export FRONTEND_APP_IMAGE=\"$FRONTEND_APP_IMAGE\"|g" /etc/environment
fi
