...
PARAMETER_PATH="/application/web/feedback"
if [ -z ${TRAEFIK_IMAGE+x} ]; then export TRAEFIK_IMAGE="none"; fi
if [ -z ${FEEDBACK_APP_IMAGE+x} ]; then export FEEDBACK_APP_IMAGE="none"; fi

# Install dependencies
sudo dnf -y update && sudo dnf install -y aws-cli jq

OUTPUT_FILE="/var/docker/feedback.env"
...
for full_name in $PARAM_NAMES; do
    ...
    if [ "$key" == "docker_images" ]; then
        ...
        FEEDBACK_APP_IMAGE=$(echo "$docker_json" | jq -r '.["feedback-application"]')
        TRAEFIK_IMAGE=$(echo "$docker_json" | jq -r '.["traefik"]')

        printf 'DOCKER_IMAGE_FEEDBACK_APPLICATION="%s"\n' "$FEEDBACK_APP_IMAGE" >> "$OUTPUT_FILE"
...
# Pull the image
sudo docker pull "$FEEDBACK_APP_IMAGE"
...
# update app version
if [ "$FEEDBACK_APP_IMAGE" != "$set_app_image" ]; then
  sudo yq -i ".services.blue-web.image = \"$FEEDBACK_APP_IMAGE\"" /var/docker/compose.yml
  sudo sed -i "s|export FEEDBACK_APP_IMAGE=.*|export FEEDBACK_APP_IMAGE=\"$FEEDBACK_APP_IMAGE\"|g" /etc/environment
fi
