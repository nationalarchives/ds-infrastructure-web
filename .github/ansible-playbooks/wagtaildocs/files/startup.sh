...
for full_name in $PARAM_NAMES; do
    ...
    if [ "$key" == "docker_images" ]; then
        ...
        WAGTAILDOCS_APP_IMAGE=$(echo "$docker_json" | jq -r '.["wagtaildocs-application"]')
        TRAEFIK_IMAGE=$(echo "$docker_json" | jq -r '.["traefik"]')

        printf 'DOCKER_IMAGE_WAGTAILDOCS_APPLICATION="%s"\n' "$WAGTAILDOCS_APP_IMAGE" >> "$OUTPUT_FILE"
...
# Pull the image
sudo docker pull "$WAGTAILDOCS_APP_IMAGE"
...
# Update app version
if [ "$WAGTAILDOCS_APP_IMAGE" != "$set_app_image" ]; then
  sudo yq -i ".services.blue-web.image = \"$WAGTAILDOCS_APP_IMAGE\"" /var/docker/compose.yml
  sudo sed -i "s|export WAGTAILDOCS_APP_IMAGE=.*|export WAGTAILDOCS_APP_IMAGE=\"$WAGTAILDOCS_APP_IMAGE\"|g" /etc/environment
fi
