...
if [ -z ${WAGTAILDOCS_APP_IMAGE+x} ]; then export WAGTAILDOCS_APP_IMAGE="none"; fi

# Install dependencies
sudo dnf -y update && sudo dnf install -y aws-cli jq

AWS_REGION="eu-west-2"
PARAMETER_PATH="/application/web/wagtaildocs"
...
OUTPUT_FILE="/var/docker/wagtaildocs.env"
...
exp_traefik_image=$(aws ssm get-parameter --name /application/web/wagtaildocs/docker_images --query Parameter.Value --region $region --output text | jq -r '.["traefik"]')
exp_app_image=$(aws ssm get-parameter --name /application/web/wagtaildocs/docker_images --query Parameter.Value --region $region --output text | jq -r '.["wagtaildocs-application"]')
...
# update app version
if [ "$WAGTAILDOCS_APP_IMAGE" != "$exp_app_image" ] || [ "$set_app_image" != "$exp_app_image" ]; then
  sudo yq -i ".services.blue-web.image = \"$exp_app_image\"" /var/docker/compose.yml
  export WAGTAILDOCS_APP_IMAGE="$exp_app_image"
  sudo sed -i "s|export WAGTAILDOCS_APP_IMAGE=.*|export WAGTAILDOCS_APP_IMAGE=\"$exp_app_image\"|g" /etc/environment
fi
