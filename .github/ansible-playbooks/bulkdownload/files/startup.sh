...
PARAMETER_PATH="/application/web/bulkdownload"
if [ -z ${TRAEFIK_IMAGE+x} ]; then export TRAEFIK_IMAGE="none"; fi
if [ -z ${BULKDOWNLOAD_APP_IMAGE+x} ]; then export BULKDOWNLOAD_APP_IMAGE="none"; fi

# Install dependencies
sudo dnf -y update && sudo dnf install -y aws-cli jq

OUTPUT_FILE="/var/docker/bulkdownload.env"
...
for full_name in $PARAM_NAMES; do
    ...
    if [ "$key" == "docker_images" ]; then
        ...
        BULKDOWNLOAD_APP_IMAGE=$(echo "$docker_json" | jq -r '.["bulkdownload-application"]')
        TRAEFIK_IMAGE=$(echo "$docker_json" | jq -r '.["traefik"]')

        printf 'DOCKER_IMAGE_BULKDOWNLOAD_APPLICATION="%s"\n' "$BULKDOWNLOAD_APP_IMAGE" >> "$OUTPUT_FILE"
...
# Pull the image
sudo docker pull "$BULKDOWNLOAD_APP_IMAGE"
...
# update app version
if [ "$BULKDOWNLOAD_APP_IMAGE" != "$set_app_image" ]; then
  sudo yq -i ".services.blue-web.image = \"$BULKDOWNLOAD_APP_IMAGE\"" /var/docker/compose.yml
  sudo sed -i "s|export BULKDOWNLOAD_APP_IMAGE=.*|export BULKDOWNLOAD_APP_IMAGE=\"$BULKDOWNLOAD_APP_IMAGE\"|g" /etc/environment
fi
