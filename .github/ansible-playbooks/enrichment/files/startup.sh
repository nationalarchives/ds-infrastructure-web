...
# Pull the image
sudo docker pull "$ENRICHMENT_APP_IMAGE"
...
# Update app version
if [ "$ENRICHMENT_APP_IMAGE" != "$set_app_image" ]; then
    sudo yq -i ".services.blue-web.image = \"$ENRICHMENT_APP_IMAGE\"" /var/docker/compose.yml
    sudo sed -i "s|export ENRICHMENT_APP_IMAGE=.*|export ENRICHMENT_APP_IMAGE=\"$ENRICHMENT_APP_IMAGE\"|g" /etc/environment
fi
