...
for full_name in $PARAM_NAMES; do
    ...
    if [ "$key" == "docker_images" ]; then
        ...
        SEARCH_APP_IMAGE=$(echo "$docker_json" | jq -r '.["search-application"]')
        TRAEFIK_IMAGE=$(echo "$docker_json" | jq -r '.["traefik"]')

        printf 'DOCKER_IMAGE_SEARCH_APPLICATION="%s"\n' "$SEARCH_APP_IMAGE" >> "$OUTPUT_FILE"
...
# Pull the image
sudo docker pull "$SEARCH_APP_IMAGE"
...
# Update app version
if [ "$SEARCH_APP_IMAGE" != "$set_app_image" ]; then
  sudo yq -i ".services.blue-web.image = \"$SEARCH_APP_IMAGE\"" /var/docker/compose.yml  
  sudo sed -i "s|export SEARCH_APP_IMAGE=.*|export SEARCH_APP_IMAGE=\"$SEARCH_APP_IMAGE\"|g" /etc/environment
fi
