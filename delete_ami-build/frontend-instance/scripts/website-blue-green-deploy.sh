...
if ! sudo docker run --net $TRAEFIK_NETWORK --rm curlimages/curl:8.00.1 --fail --silent "$HEALTH_CHECK_URL" >/dev/null; then
  ...
  sudo /usr/local/bin/docker-compose --file /var/docker/compose.yml stop --timeout=30 $INACTIVE_SERVICE
  exit 1
fi
...
if [[ -n "$ACTIVE_SERVICE" ]]; then
  ...
  sudo /usr/local/bin/docker-compose --file /var/docker/compose.yml stop --timeout=30 $ACTIVE_SERVICE
fi
